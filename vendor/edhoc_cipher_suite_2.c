 ...
 *
 * ---------------------------------------------------------------------------
 * VENDORED into usb_ncm_net from libedhoc v1.14.1
 * (deps/modules/lib/libedhoc/helpers/src/edhoc_cipher_suite_2.c), with one
 * local patch: edhoc_cipher_suite_2_key_agreement() obtains its
 * mbedtls_ecp_group directly instead of via the mbedtls_pk_ec() accessor, which
 * this Zephyr's mbedTLS (tf-psa-crypto 4.x) removed. See the "VENDOR PATCH"
 * marker below and docs/edhoc-oscore-plan.md. Re-sync if the upstream helper
 * changes. Everything else is byte-identical to upstream.
 * ---------------------------------------------------------------------------
 */
...
/* mbedTLS headers: legacy ECP is exposed via mbedtls/ecp.h when
 * CONFIG_MBEDTLS_DECLARE_PRIVATE_IDENTIFIERS=y (see oscore.conf). The upstream
 * <mbedtls/pk.h> include is dropped -- the patched key agreement no longer uses
 * the classic PK API. */
				       ...
				       size_t *shr_sec_len)
{
	...
	/*
	 * VENDOR PATCH (usb_ncm_net): upstream obtains the mbedtls_ecp_group by
	 * standing up an mbedtls_pk_context and reaching into it via
	 * mbedtls_pk_ec(). mbedTLS 4.x (tf-psa-crypto) removed mbedtls_pk_ec (PK
	 * contexts no longer wrap an mbedtls_ecp_keypair), so load a standalone
	 * group instead -- the only thing upstream used the PK context for.
	 */
	...
	ret = mbedtls_ecp_decompress(&grp, peer_pub_key, peer_pub_key_len,
				     decom_pub_key, ARRAY_SIZE(decom_pub_key),
				     &decom_pub_key_len);
